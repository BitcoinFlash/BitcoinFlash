...
A Linux bash script that will set up traffic control (tc) to limit the outgoing bandwidth for connections to the Bitcoin Flash network. This means one can have an always-on bitcoinflashd instance running, and another local bitcoinflashd/bitcoinflash-qt instance which connects to this node and receives blocks from it.
...
Contains files used to package bitcoinflashd/bitcoinflash-qt
